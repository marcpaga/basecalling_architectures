...
class TimeAnalysisModel(BaseModelImpl):

    def __init__(self, cnn_type, rnn_size, rnn_type, num_layers = 2, bidirectional = True, *args, **kwargs):
        super(TimeAnalysisModel, self).__init__(*args, **kwargs)
    ...
    def _build_rnn(self):
        ...
        if self.rnn_type == 'lstm':
            ...
            if self.bidirectional:
                self.rnn_out_size = int(self.rnn_size * 2)
            else:
                self.rnn_out_size = self.rnn_size
            ...
            if self.bidirectional:
                ...
                self.rnn_out_size = self.rnn_size
        elif self.rnn_type == 'urnano':

            self.rnn_out_size = 128
            rnn = nn.GRU(self.cnn_out_size, hidden_size = 64, num_layers = 3, bidirectional = True)

        elif self.rnn_type == 'catcaller':
            
            self.rnn_out_size = 512

            d_model = self.cnn_out_size
            d_ff = 512
            dropout = 0.1
            padding = 1
            kernel = 3
        ...
        elif self.rnn_type == 'sacall':
            
            self.rnn_out_size = self.cnn_out_size
            d_model = self.cnn_out_size
            dropout = 0.1
            n_layers = 6
            n_head = 8
        ...
        elif self.rnn_type == 'bonito':
            self.rnn_out_size = 384
            rnn = nn.Sequential(
                BonitoLSTM(self.cnn_out_size, 384, reverse = True),
                BonitoLSTM(384, 384, reverse = False),
                BonitoLSTM(384, 384, reverse = True),
                BonitoLSTM(384, 384, reverse = False),
                BonitoLSTM(384, 384, reverse = True)
            )
        elif self.rnn_type == 'bonitorev':
            self.rnn_out_size = 384
            rnn = nn.Sequential(
                BonitoLSTM(self.cnn_out_size, 384, reverse = False),
                BonitoLSTM(384, 384, reverse = True),
                BonitoLSTM(384, 384, reverse = False),
                BonitoLSTM(384, 384, reverse = True),
                BonitoLSTM(384, 384, reverse = False)
    ...
    def _build_decoder(self):
        ...
        return nn.Sequential(nn.Linear(self.rnn_out_size, 5), nn.LogSoftmax(-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rnn-type", type=str, choices=[
        'lstm', 
        'gru', 
        'urnano', 
        'catcaller', 
    ...
    # output stuff
    output_dir = os.path.join('/hpc/compgen/projects/nanoxog/babe/analysis/mpages/models/time_analysis', args.task)
    output_dir += '/'
